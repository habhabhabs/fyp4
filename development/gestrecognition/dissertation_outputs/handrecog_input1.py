...
while True:
    try:  
        ...
        # define region of interest (detection area) step 1
        roi = frame[100:300, 100:300]
        cv2.rectangle(frame, (100, 100), (300, 300), (0, 255, 0), 0)
         
        # define range of skin colour in HSV (Hue Saturation Values) step 2 
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
        lower_skin = np.array([0, 20, 70], dtype = np.uint8)
        upper_skin = np.array([20, 255, 255], dtype = np.uint8)
        
        # check if skin colour defined matches lower_skin and upper_skin bounds step 3  
